...
var fires_int = ee.FeatureCollection('users/<user>/...burn_bndy') // Edit path here

// This function computes the feature's geometry area and adds it as a property.
var added_attributes = function(feature) {
  return feature.set({NAME: 'BB Complex (Booth)', Fire_ID: 'OR4451112177120030819', Fire_Year: 2003, Start_Day: 231, End_Day: 242}); // Populate with fire’s name, ID, year, and Julian start/end day
};

// Map the area getting function over the FeatureCollection.
var fires = fires_int.map(added_attributes);
...
var bandsToExport      = ['CBI', 'CBI_bc', 'rbr'] // Edit as desired
...
var cbi = ee.FeatureCollection("users/grumpyunclesean/CBI_predictions/data_for_ee_model"); // Request access from owner
...
var ls8_Indices = function(lsImage){
  ...
  var evi  = lsImage.expression(
              ...
              {'SR_B5': lsImage.select('SR_B5').multiply(0.0001),
              'SR_B4': lsImage.select('SR_B4').multiply(0.0001),
              'SR_B2': lsImage.select('SR_B2').multiply(0.0001), 
              }).toFloat();
  var mirbi = lsImage.expression(
              '((10 * SR_B6) - (9.8 * SR_B7) + 2)',
              {'SR_B6': lsImage.select('SR_B6').multiply(0.0001),
              'SR_B7': lsImage.select('SR_B7').multiply(0.0001),
              }).toFloat();              
...
var ls4_7_Indices = function(lsImage){
  ...
  var evi = lsImage.expression(
              ...
              {'SR_B4': lsImage.select('SR_B4').multiply(0.0001),
              'SR_B3': lsImage.select('SR_B3').multiply(0.0001),
              'SR_B1': lsImage.select('SR_B1').multiply(0.0001),
              }).toFloat();
  var mirbi = lsImage.expression(
...
////////////////////////////////////////////////
function lsCfmask(img) {
  var qa   = img.select('QA_PIXEL');          // C-02 quality band
  var clearMask = qa.bitwiseAnd(1 << 3).eq(0)   // clouds
...
var waterMask = ee.Image('UMD/hansen/global_forest_change_2022_v1_9').select(['datamask']).eq(1);
...
//************ User should set their own short ID so exported files are named correctly (e.g. 'seeleyr') ***********//
var onid = '<user>' // Specify export location for datasets, change as desired but ensure that there is a folder named 'cbi'
...
for (var j = 0; j < nFires; j++){
  ...
  for (var i = 0; i < nBands; i++) {
    ...
    Export.image.toDrive({
      ...
      description: onid + '_' + bandExport,
      fileNamePrefix: onid + '_' + bandExport,
      maxPixels: 1e13,
      scale: 30,
      crs: "EPSG:4326",
      folder: 'cbi',     
...
/******************** OPTIONAL LOCAL ANALYSIS (Hogg Pass) *******************
 * Computes mean CBI_bc inside a 14-km buffer around Hogg Pass SNOTEL       *
 * If CBI layers are missing (e.g. user did not train RF), it falls back    *
   to mean dNBR just so the demo runs                                       *
 ***************************************************************************/

// Geometry
var huc8  = ee.FeatureCollection('USGS/WBD/2017/HUC08') // Edit watershed here
...
var region14km = snotel.buffer(14000).intersection(huc8); // Edit radius here
Map.addLayer(region14km, {}, '14-km buffer');

// Choose an existing band name
var outBand = indices.first().bandNames()
                 .contains('CBI_bc')
                 .evaluate(function(hasCBI){
   var metric = hasCBI ? 'CBI_bc' : 'dnbr'; // fallback if no CBI
   var merged = indices.mosaic().select(metric).clip(region14km);

   // Summary
   var mean = merged.reduceRegion({
     reducer: ee.Reducer.mean(),
     geometry: region14km,
     scale: 30
   }).get(metric);
   print(['Mean ' + metric + ' in buffer:', mean]);
   ...
   var hist = ui.Chart.image.histogram({
     ...
     title: metric + ' histogram (buffer)',
     hAxis: {title: metric},
     vAxis: {title: 'frequency'}
   });
   print(hist);

   // Binary mask (> threshold if using CBI_bc, else just show dnbr > 0)
   var thresh = hasCBI ? 0.99 : 0;
   var hot    = merged.gt(thresh);
   Map.addLayer(hot, {palette:['red']}, metric + ' > ' + thresh);
});
